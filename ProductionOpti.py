...
def CreateIniFilesPhase1 (BotName ="MACD Sample" ,PairList ='EURUSD',TimeFrameList ='H1',OptimizationCriterionList=6,
    ...
    'Report=reports\{}\{}\{}\OptiResults-{}-{}-{}-Phase{}'.format(BotName.get(),PairList,TimeFrameList,BotName.get(),
                                                                  PairList,TimeFrameList,Phase) + "\n" \
    ...
    for i in PairList:
        for j in TimeFrameList:
            try:
                ...
                dfback = movecol(dfback, cols_to_move=['Absolute DD'], ref_col='Equity DD %', place='After')
                ...
                dfforward = movecol(dfforward, cols_to_move=['Forward Absolute DD'], ref_col='Equity DD %',place='After')
                ...
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Result'], ref_col='Result', place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Profit'], ref_col='Profit', place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Expected Payoff'], ref_col='Expected Payoff',place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Profit Factor'], ref_col='Profit Factor',place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Recovery Factor'], ref_col='Recovery Factor',place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Sharpe Ratio'], ref_col='Sharpe Ratio',place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Custom'], ref_col='Custom', place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Average Loss'], ref_col='Average Loss',place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Equity DD %'], ref_col='Equity DD %',place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Absolute DD'], ref_col='Absolute DD',place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Trades'], ref_col='Trades', place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Win Ratio'], ref_col='Win Ratio',place='After')
                Complete_df = movecol(Complete_df, cols_to_move=['Forward Lots'], ref_col='Lots', place='After')
                ...
                for index, row in Complete_df.iterrows():
                    ...
                    try:
                        if (row['Profit'] >= int(FilterNetProfitPhase1.get())) and (
                                row['Expected Payoff'] >= float(FilterExpectedPayoffPhase1.get())) and (
                                row['Profit Factor'] >= float(FilterProfitFactorPhase1.get())) and (
                                row['Custom'] >= float(FilterCustomPhase1.get())) and (
                                (row['Absolute DD']) <= float(FilterEquityDDPhase1.get()) + 100) and (
                                row['Trades'] >= int(FilterTradesPhase1.get())) and \
                                (row['Forward Profit'] >= int(ForwardFilterNetProfitPhase1.get())) and (
                                row['Forward Expected Payoff'] >= float(ForwardFilterExpectedPayoffPhase1.get())) and (
                                row['Forward Profit Factor'] >= float(ForwardFilterProfitFactorPhase1.get())) and (
                                row['Forward Custom'] >= float(ForwardFilterCustomPhase1.get())) and (
                                (row['Forward Absolute DD']) <= float(ForwardFilterEquityDDPhase1.get()) + 100) and (
                                row['Forward Trades'] >= int(ForwardFilterTradesPhase1.get())):
                            Projectcount += 1
                            pass
                        else:
...
def CreateIniFilesPhase2 (BotName ="MACD Sample" ,PairList ='EURUSD',TimeFrameList ='H1',OptimizationCriterionList=6,
                          ModelList=2,OptimizationList=2,ShutdownTerminalList=1,VisualList=0,LeverageValue=33,
    ...
    'ForwardMode={}'.format(ForwardModeList) + "\n" \
    'ForwardDate={}'.format(ForwardDate.get()) + "\n" \
    'Report=reports\{}\{}\{}\OptiResults-{}-{}-{}-Phase{}'.format(BotName.get()
                                                                  ,PairList,TimeFrameList,BotName.get(),PairList,
                                                                  TimeFrameList,Phase) + "\n" \
    ';--- If the specified report already exists, it will be overwritten' + "\n" \
    'ReplaceReport={}'.format(ReplaceReportList) + "\n" \
    ';--- Set automatic platform shutdown upon completion of testing/optimization' + "\n" \
...
def HillClimbPhase2():  # Hill Climbing Step
    ...
    while HCCount < HCLaps:
            ...
            with open(HCOptiset, "r+") as f4:
                ...
                for line in HCtext2:
                    ...
                    if (line[0] != ';' or line[0] == ' ') and line[-2] == 'Y':
                        varname = line.split('=')
                        varline = varname[1].split('||')
                        #print('this is the running variable:',v)
                        if varname[0] == v:
                            #print(varline[0])
                            #print(vBestResult)
        ...
        #else:
            #print('Done for a total of ',HCCount,' laps.')
            ...
            try:
                ...
                with open(file_a,'r',encoding='utf-16') as f:
                    ...
                    for g,row in dfOpti.iterrows():
                        ...
                        CreateIniFilesPhase3(PairList=i,TimeFrameList=j,TailNumber=g)
...
def CreateIniFilesPhase3 (PairList ='EURUSD',TimeFrameList ='H1',OptimizationCriterionList=0,ModelList=2,OptimizationList=0,ShutdownTerminalList=1,VisualList=0,LeverageValue=33,ReplaceReportList=1,UseLocalList=1,ForwardModeList=0,ExecutionValue=28,Phase=3,TailNumber=0):
    ...
    '[Tester]' + "\n" \
    'Expert=Advisors\{}'.format(BotName.get()) + "\n" \
    'ExpertParameters=\{}\Phase3-{}-{}-{}-{}.set'.format(BotName.get(),BotName.get(),PairList,TimeFrameList,TailNumber) + "\n" \
    'Symbol={}'.format(PairList) + 'MT5' + "\n" \
    'Period={}'.format(TimeFrameList) + "\n" \
    ';Login=XXXXXX' + "\n" \
    'Model={}'.format(ModelList) + "\n" \
    ...
    ';ForwardMode={}'.format(ForwardModeList) + "\n" \
    ';ForwardDate={}'.format(ForwardDate.get()) + "\n" \
    'Report=reports\{}\SETS\Phase3-{}-{}-{}-{}'.format(BotName.get(),BotName.get(),PairList,TimeFrameList,TailNumber) + "\n" \
    ';--- If the specified report already exists, it will be overwritten' + "\n" \
    'ReplaceReport={}'.format(ReplaceReportList) + "\n" \
    ';--- Set automatic platform shutdown upon completion of testing/optimization' + "\n" \
    'ShutdownTerminal={}'.format(ShutdownTerminalList) + "\n" \
